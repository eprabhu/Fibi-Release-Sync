...
BEGIN
    ...
    -- QUESTION_NUMBER will different in across questionnaire.
        ...
        IF LI_QUES_ANS_HEADER_ID IS NOT NULL THEN
            ...
            AND T2.QUESTION_NUMBER = 6912; -- Q1. Does this agreement or award already have an SAP Grant#?
             ...
             IF LS_Q1_ANSWER <> '' AND LS_Q1_ANSWER IS NOT NULL AND UPPER(LS_Q1_ANSWER) = UPPER('Yes - That is a Modification of Funded Agreement')  THEN
                ...
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6918; -- Q1 -> yes -> Please search for and choose the relevant Award.
				
				SELECT TRIM(T1.ANSWER) INTO LS_MOD_TYPE FROM QUEST_ANSWER T1
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6980; -- Q1 -> yes -> Please Select Modification Type.
            ...
            IF LS_Q1_ANSWER IS NULL OR LS_Q1_ANSWER = '' OR UPPER(LS_Q1_ANSWER) = 'NO' THEN
                ...
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6913; -- Q1.2 Was a Fibi Proposal Development record (PD) submitted for this agreement/award?
                ...
                IF LS_Q2_ANSWER <> '' AND LS_Q2_ANSWER IS NOT NULL AND UPPER(LS_Q2_ANSWER) = UPPER('Yes - Select IPN')  THEN
                    ...
                    INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                    WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                    AND T2.QUESTION_NUMBER = 6917; -- Q1.2 -> yes -> Please search for and choose the relevant IPN.
				...
				IF LS_Q2_ANSWER <> '' AND LS_Q2_ANSWER IS NOT NULL AND UPPER(LS_Q2_ANSWER) = UPPER('No - You are initiating an Unfunded Agreement. (​If this is a Funded Agreement, please submit a Fibi PD and obtain a Fibi IPN before submitting this Agreement)')  THEN
					...
					INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
					WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
					AND T2.QUESTION_NUMBER = 6914; -- Q1.3  Is this Agreement related to a current Agreement?​
					...
					IF LS_Q3_ANSWER <> '' AND LS_Q3_ANSWER IS NOT NULL AND UPPER(LS_Q3_ANSWER) = UPPER(TRIM('No - This is a New Unfunded Agreement')) THEN
							...
							INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
							WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                            AND T2.QUESTION_NUMBER IN ( 6985,6983);
							...
							IF LS_Q4_ANSWER <> '' AND LS_Q4_ANSWER IS NOT NULL AND UPPER(LS_Q4_ANSWER) = 'YES' THEN
									...
									INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
									WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID									
                                    AND T2.QUESTION_NUMBER IN ( 6986,6984)
                                    AND T1.ANSWER_LOOKUP_CODE IS NOT NULL; -- Q1.2 -> yes -> Please search for and choose the relevant IPN.
						...
						IF LS_Q3_ANSWER <> '' AND LS_Q3_ANSWER IS NOT NULL AND UPPER(LS_Q3_ANSWER) = UPPER(TRIM('Yes - This is a Modification of an Unfunded Agreement')) THEN 
									...
									INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
									WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
									AND T2.QUESTION_NUMBER IN ( 6996,6995)
